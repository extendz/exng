...
export class NgDatabaseFactory {
  ...
  open(dbName: string, version?: number): Observable<OpenConnectionResponse> {
    return new Observable((observer) => {
      let upgraded = false;
      const DBOpenRequest: IDBOpenDBRequest = this.factory.open(
        dbName,
        version
      );
  ...
  createDataBases(databases: NgDataBase[]): Observable<NgConnection> {
    ...
    return forkJoin(operators).pipe(map(mappingResponse));
  }
  /**
  ...
  private createConnection(
    ...
    tables: NgStore[],
    version?: number
  ): Observable<NgConnection> {
    return this.open(dbName, version).pipe(
      map((data) => ({
        [dbName]: new IndexedDatabase(data.result, data.upgraded, tables),
      }))
    );
  }
}
