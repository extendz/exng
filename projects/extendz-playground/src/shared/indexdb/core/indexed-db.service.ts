...
export class IndexedDB {
   ...
   */
  get onReady() {
    return this.databaseFactory.connectionsReady.pipe(skipWhile((event: boolean) => !event));
  }
  ...
  list<M = any>(storeName: string, databaseName?: string): Observable<M[]> {
    return this.onReady.pipe(
      switchMap(() => this.databaseFactory.getDatabase(databaseName).list<M>(storeName))
    );
  }
  /**
  ...
  get<M = any>(storeName: string, key: any, databaseName?: string): Observable<M> {
    return this.onReady.pipe(
      switchMap(() => this.databaseFactory.getDatabase(databaseName).get<M>(storeName, key))
    );
  }
  /**
  ...
  create<M = any>(storeName: string, data: M, databaseName?: string): Observable<M> {
    ...
    return this.onReady.pipe(
      switchMap(() => this.databaseFactory.getDatabase(databaseName).create<M>(storeName, data))
    );
  }
  /**
  ...
  update<M = any>(storeName: string, data: M, databaseName?: string): Observable<M> {
    return this.onReady.pipe(
      switchMap(() => this.databaseFactory.getDatabase(databaseName).update<M>(storeName, data))
    );
  }
  /**
  ...
  delete(storeName: string, key: IDBValidKey, databaseName?: string): Observable<boolean> {
    return this.onReady.pipe(
      switchMap(() => this.databaseFactory.getDatabase(databaseName).delete(storeName, key))
    );
  }
}
